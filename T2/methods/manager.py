...
from dataloaders.sampler import data_sampler_CFRL as data_sampler
...
class Manager(object):
    ...
    def get_proto(self, args, encoder, mem_set):
        ...
        for step, batch_data in enumerate(data_loader):
            labels, tokens, ind, mask = batch_data
            tokens = torch.stack([x.to(args.device) for x in tokens], dim=0)
            mask = torch.stack([x.to(args.device) for x in mask], dim=0)

            with torch.no_grad():
                feature, rep= encoder.bert_forward(tokens, mask)
    ...
    def select_data(self, args, encoder, sample_set):
        ...
        for step, batch_data in enumerate(data_loader):
            ...
            mask = torch.stack([x.to(args.device) for x in mask], dim=0)
            with torch.no_grad():
                feature, rp = encoder.bert_forward(tokens, mask)
    ...
    def train_simple_model(self, args, encoder, training_data, epochs):
        ...
        def train_data(data_loader_, name = "", is_mem = False):
            ...
            for step, batch_data in enumerate(td):
                ...
                hidden, reps = encoder.bert_forward(tokens, mask)
    ...
    def train_mem_model(self, args, encoder, mem_data, proto_mem, epochs, seen_relations):
        ...
        def train_data(data_loader_, name = "", is_mem = False):
            ...
            for step, batch_data in enumerate(td):
                ...
                zz, reps = encoder.bert_forward(tokens, mask)
                hidden = reps
    ...
    def evaluate_strict_model(self, args, encoder, test_data, protos4eval, featrues4eval, seen_relations):
        ...
        for step, batch_data in enumerate(data_loader):
            ...
            seen_sim = torch.gather(logits, 1, torch.tensor(seen_relation_ids, device=args.device).unsqueeze(0).expand(logits.size(0), -1))
    ...
    def train(self, args):
        ...
        for i in range(args.total_round):
            ...
            sampler = data_sampler(config=args, seed=args.seed+i*100)
