...
from models.encoder import Bert_Encoder
...
class RelTrainner(Trainer):
    ...
    def get_proto(self, data):
        ...
        for step, batch_data in enumerate(data_loader):
            _, tokens = batch_data
            for key in tokens:
                tokens[key] = tokens[key].to(config.device)
            with torch.no_grad():
    ...
    def train_mem_model(self, data, new_relation_data, num_of_epochs=10):
        ...
        for epoch_i in range(num_of_epochs):
            losses = []
            for step, (labels, tokens) in enumerate(tqdm(data_loader)):
                optimizer.zero_grad()
                logits_all = []
                ...
                if self.prev_encoder is not None:
                    ...
                    feature_distill_loss = distill_criterion(normalized_reps_emb, normalized_prev_reps_emb,
                                                            torch.ones(tokens['input_ids'].size(0)).to(
                                                                config.device))
    ...
    def train_base(self, data):
        ...
        for epoch_i in range(self.config.step1_epochs):
            ...
            for step, batch_data in enumerate(tqdm(data_loader)):
                ...
                for key in tokens.keys():
                    tokens[key] = tokens[key].to(self.config.device)
                reps = self.encoder(tokens)
                reps, _ = self.dropout_layer(reps)
                logits = self.classifier(reps)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pattern", default="softprompt", type=str)
    parser.add_argument("--use_unused", default=False, type=bool)
    parser.add_argument("--use_entity_marker", default=True, type=bool)
    ...
    config.num_protos = args.num_protos
    config.use_unused = args.use_unused
    config.use_entity_marker = args.use_entity_marker
