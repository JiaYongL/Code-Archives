...
class Bert_Encoder(base_model):
    ...
    def __init__(self, config):
        ...
        if not config.use_unused:
            self.encoder.resize_token_embeddings(config.vocab_size + config.prompt_length * 4 + (4 if config.use_entity_marker else 0))
    ...
    def forward(self, inputs):
        ...
        output = self.encoder(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask']).last_hidden_state
        output = output.gather(1, inputs['pos'].unsqueeze(2).expand(-1, -1, output.size(-1)))
